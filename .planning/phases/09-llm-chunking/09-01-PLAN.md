...
Ensure all cells are properly integrated after existing quality inspection cells, have unique cell IDs, and follow notebook cell execution order.
  ...
  <verify>
    <automated>jupyter nbconvert --to notebook --execute course/day1.ipynb --output /tmp/day1_test.ipynb --ExecutePreprocessor.timeout=120 --ExecutePreprocessor.allow_errors=True 2>&1 && python3 -c "import json; nb=json.load(open('/tmp/day1_test.ipynb')); cells=[c for c in nb['cells'] if c['cell_type']=='code']; errors=[c for c in cells if any('error' in o.get('output_type','') for o in c.get('outputs',[]))]; print(f'Executed {len(cells)} code cells, {len(errors)} errors'); exit(1 if len(errors)>2 else 0)"</automated>
    <manual>Verify function definitions exist: grep -c "def chunk_by_llm" course/day1.ipynb returns 1</manual>
  </verify>
  <acceptance_criteria>
    ...
    - Notebook executes without fatal errors (API key absence handled gracefully)
...
Ensure all cells have unique cell IDs, proper type hints (dict[str, Any] -> list[dict[str, Any]]), and comprehensive docstrings following project/ engineering standards.
  ...
  <verify>
    <automated>jupyter nbconvert --to notebook --execute project/owasp_homework.ipynb --output /tmp/owasp_test.ipynb --ExecutePreprocessor.timeout=120 --ExecutePreprocessor.allow_errors=True 2>&1 && python3 -c "import json; nb=json.load(open('/tmp/owasp_test.ipynb')); cells=[c for c in nb['cells'] if c['cell_type']=='code']; errors=[c for c in cells if any('error' in o.get('output_type','') for o in c.get('outputs',[]))]; print(f'Executed {len(cells)} code cells, {len(errors)} errors'); exit(1 if len(errors)>2 else 0)"</automated>
    <manual>Verify type hints exist: grep "dict\[str, Any\]" project/owasp_homework.ipynb returns matches</manual>
  </verify>
  <acceptance_criteria>
...
1. **Executable notebook verification:**
```bash
# Course notebook executes without fatal errors
jupyter nbconvert --to notebook --execute course/day1.ipynb --output /tmp/day1_verify.ipynb --ExecutePreprocessor.timeout=120 --ExecutePreprocessor.allow_errors=True

# Project notebook executes without fatal errors
jupyter nbconvert --to notebook --execute project/owasp_homework.ipynb --output /tmp/owasp_verify.ipynb --ExecutePreprocessor.timeout=120 --ExecutePreprocessor.allow_errors=True

# Verify execution completed (check output notebooks exist and have outputs)
python3 -c "import json; nb=json.load(open('/tmp/day1_verify.ipynb')); print(f'Course: {len([c for c in nb[\"cells\"] if c[\"cell_type\"]==\"code\"])} code cells')"
python3 -c "import json; nb=json.load(open('/tmp/owasp_verify.ipynb')); print(f'Project: {len([c for c in nb[\"cells\"] if c[\"cell_type\"]==\"code\"])} code cells')"
```

2. **Function presence in both notebooks:**
```bash
...
11. Both notebooks execute via jupyter nbconvert without fatal errors
</success_criteria>
